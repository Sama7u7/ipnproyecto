...
class Deshidartador1Controller extends Controller
{
    public function __construct()
    {
        // Crear la ruta para recibir los datos directamente en este controlador usando el nombre en minúsculas
        Route::post("/germinadores/deshidartador1/data", [self::class, 'receiveData'])
            ->name("germinadores.deshidartador1.data");
    }
    ...
    public function receiveData(Request $request)
    {
        ...
        DB::table("deshidartador1_dht1")->insert([
        ...
        DB::table("deshidartador1_dht2")->insert([
            'temperatura' => $temperatura,
            'humedad' => $humedad,
            'fecha_actual' => now(),
        ]);

         DB::table("deshidartador1_dht3")->insert([
            'temperatura' => $temperatura,
            'humedad' => $humedad,
            'fecha_actual' => now(),
        ]);

         DB::table("deshidartador1_peso")->insert([
            'peso' => $peso,
            'fecha_actual' => now(),
        ]);
